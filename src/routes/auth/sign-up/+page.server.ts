...
export const actions = {
    register: async ({cookies, locals, request, fetch}) => {
        const data = await request.formData();
        const email = data.get("email")?.toString();
        const password = data.get("password")?.toString();
        const langIso2 = data.get("lang_iso2")?.toString();
        const nextLocation = data.get("next")?.toString() || "/shelf";

        if (!email || !password || !langIso2) {
        ...
        try {
            const user = await getUserAPI({fetch}).register(email, password, langIso2);
